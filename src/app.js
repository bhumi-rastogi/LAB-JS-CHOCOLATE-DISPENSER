...
var target = function (chocolates, color, count) {
...
function removeChocolates(chocolates, count){
  var chocoStore = chocolates.length;
  return count <= 0
  ? 'Number cannot be zero/negative'
  : chocoStore < count
...
var helperDispenser = function (chocolates, number) {
...
function dispenseChocolates(chocolates, number) {
  const chocoStore = chocolates.length;
  return number <= 0
  ? 'Number cannot be zero/negative'
  : chocoStore < number
  ? 'Insufficient chocolates in the dispenser'
...
function dispenseRainbowChocolates(chocolates){
  ...
  countOfEachChocolate.forEach((count) => {
    if (count % 3 == 0) {
      totalNumberOfRainbowChocolates += count 
  }
  });
